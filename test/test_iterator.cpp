...
TEST(Iterator, can_make_iterator_is_not_empty) {
	List<int> ls;
	ls.append(50);
	EXPECT_NO_THROW(List<int>::Iterator it(ls.getHead()));
}
...
TEST(Iterator, check_correct_iterator_in_head) {
	...
	ls.append(50);
	ls.append(1);
	List<int>::Iterator it(ls.getHead());
...
TEST(Iterator, check_correct_iterator_in_center) {
	...
	ls.append(50);
	ls.append(1);
	List<int>::Iterator it(ls.getHead()->next);
...
TEST(Iterator, check_correct_iterator_in_tail) {
	...
	ls.append(50);
	ls.append(1);
	List<int>::Iterator it(ls.getTail());
...
TEST(Iterator, check_correct_operator_compare_false) {
	...
	ls.append(1);
	List<int>::Iterator it(ls.getTail());
	List<int>::Iterator itl(ls.getHead());
	EXPECT_FALSE(it == itl);
}
